...
<script>
    ...
    function lancerRecherche() {
        const val = normaliser(document.getElementById('input-recherche').value.trim());
        const rows = document.querySelectorAll('tbody tr');
        rows.forEach(row => {
            if (row.cells.length <= 1) return; // ligne "aucune commande"
            const texte = normaliser(row.textContent);
            row.style.display = (!val || texte.includes(val)) ? '' : 'none';
        });
        document.getElementById('btn-reset-recherche').classList.toggle('d-none', !val);
        mettreAJourBouton();
    }

    document.getElementById('btn-recherche').addEventListener('click', lancerRecherche);
    ...
    document.querySelectorAll('th.sortable').forEach(th => {
        th.addEventListener('click', function () {
            ...
            document.querySelectorAll('th.sortable').forEach(h => {
                ...
                icon.className = 'bi bi-filter sort-icon';
                h.style.color = '';
            });
            const activeIcon = this.querySelector('.sort-icon');
            activeIcon.className = sortAsc ? 'bi bi-sort-down-alt sort-icon' : 'bi bi-sort-up-alt sort-icon';
            this.style.color = 'var(--bs-primary, #0d6efd)';
            ...
            // Réinitialiser les cases après le tri
            mettreAJourBouton();
        });
    });
                ...
                <div class="card">
                    ...
                    <div class="card-body border-bottom pb-3">
                        <div class="input-group">
                            ...
                            <input type="text" id="input-recherche" class="form-control" placeholder="Rechercher une commande...">
                            <button class="btn btn-primary" id="btn-recherche" type="button">Rechercher</button>
                            <button class="btn btn-outline-secondary d-none" id="btn-reset-recherche" type="button" title="Effacer"><i class="bi bi-x-lg"></i></button>
                        </div>
                    </div>
                        ...
                        <table class="table table-hover">
                            <thead>
                                <tr class="text-center">
                                    <th class="sortable" data-col="0" style="cursor:pointer; user-select:none;">ID <i class="bi bi-filter sort-icon"></i></th>
                                    <th class="sortable" data-col="1" style="cursor:pointer; user-select:none;">Société <i class="bi bi-filter sort-icon"></i></th>
                                    <th class="sortable" data-col="2" style="cursor:pointer; user-select:none;">N° Commande Client <i class="bi bi-filter sort-icon"></i></th>
                                    <th>Actions</th>
                                    <th style="width: 40px;">
